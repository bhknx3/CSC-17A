...
class Deck {
    ...
    public:
        ...
        ~Deck();
        //Accessors
        int getNCrd() const;    //Get number of cards in deck
        T *getDeck() const;     //Get pointer to deck
        //Mutators;
        void setNCrd(int);      //Set number of cards
        void setPtr(T *);       //Set pointer to card
        //Functions
        void shuffle();         //Shuffle deck (using algorithm)
        void shuffle(int);      //Shuffle deck
        void dspDeck();         //Print deck
};
...
//Set pointer to cards
template <class T>
void Deck<T>::setPtr(T *p) {
    card = p;
}   
