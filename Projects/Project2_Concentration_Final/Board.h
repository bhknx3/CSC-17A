...
class Board {
    ...
    public:
        ...
        //Constructor
        Board();
        Board(const T *, const int, const int);
        //Destructor
        ~Board();
        //Accessors
        ...
        T * getIndx(const int, const int) const; //Get address of certain element
        //Mutators
...
//Constructor 2
template <class T>
Board<T>::Board(const T *cards, const int row, const int col) {
...
template <class T>
T * Board<T>::getIndx(const int r, const int c) const {
    return &index[r][c];
}
